package src.checkersGames.standardCheckers.moveRules.specialRules;
...
public class ManEatingMove implements MoveRule {
    ...
    private final IsMovingForward isMovingForward = new IsMovingForward();
    ...
    private boolean checkInitialConditions(Tile origin, Tile destination, List<GameState> gameStates){
        if(!movementInitialConditions(origin, destination, gameStates))
            return false;
    ...
    private boolean movementInitialConditions(Tile origin, Tile destination, List<GameState> gameStates){
        ...
        if (isDiagonalMove.isValidMove(origin, destination,gameStates) == MoveType.INVALID)
            return false;
        if(isMovingForward.isValidMove(origin, destination,gameStates) == MoveType.INVALID)
            return false;
        return true;
    }
    ...
    private MoveType arePossibleAttacksHelper(Tile realOrigin,Tile origin, Tile destination, List<GameState> gameStates){
        if(checkPossibleAttacksMovementRules(realOrigin,origin,destination,gameStates) == MoveType.INVALID)
            return MoveType.INVALID;
        if (isDestinationTileFree.isValidMove(origin, destination,gameStates) == MoveType.INVALID)
            return MoveType.INVALID;
    ...
    private MoveType checkPossibleAttacksMovementRules(Tile realOrigin,Tile origin, Tile destination, List<GameState> gameStates){
        if (isMovingInYAxisByMaxNTiles.isValidMove(origin, destination,gameStates) == MoveType.INVALID)
            return MoveType.INVALID;
        if (isMovingInYAxisByMinNTiles.isValidMove(origin, destination,gameStates) == MoveType.INVALID)
            return MoveType.INVALID;
        if (isDiagonalMove.isValidMove(origin, destination,gameStates) == MoveType.INVALID)
            return MoveType.INVALID;
        if(isMovingForwardHelper(realOrigin, origin, destination, gameStates) == MoveType.INVALID)
            return MoveType.INVALID;
        return MoveType.SPECIAL1;
    }
    private MoveType isMovingForwardHelper(Tile realOrigin, Tile origin, Tile destination, List<GameState> gameStates) {
        TeamColor pieceColor = gameStates.get(gameStates.size() - 1).getBoard().getPiece(realOrigin).getColor();
