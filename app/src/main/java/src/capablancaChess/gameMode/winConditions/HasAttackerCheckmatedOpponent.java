package src.capablancaChess.gameMode.winConditions;

import src.capablancaChess.gameMode.gameRules.IsPlayerNotInCheck;
...
public class HasAttackerCheckmatedOpponent implements WinCondition {
    CapablancaGameStateFactory gameStateFactory = new CapablancaGameStateFactory();
    @Override
    public boolean isGameWonByPlayer(Player attackingPlayer, List<GameState> gameStates) {
        ...
        if(new src.chess.gameMode.gameRules.IsPlayerNotInCheck(defendingPlayer).isGameRuleValid(gameStates)){
            return false;
        }
        ...
        for(Tile defenderTile : defenderTiles){
            for(Tile targetTile : totalTiles){
                ...
                if(moveType != MoveType.INVALID){
                    List<GameState> auxiliarGameStates =gameStateFactory.movePiece(moveType,defenderTile, targetTile, gameStates);
